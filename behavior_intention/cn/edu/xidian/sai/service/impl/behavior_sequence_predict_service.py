...
def behavior_sequence_predict():
    ...
    num_candidates_saved = int(get_param_values_from_train_dao()['top_n'])
    parser = argparse.ArgumentParser()
    parser.add_argument('-num_layers', default=2, type=int)
    parser.add_argument('-hidden_size', default=100, type=int)
    parser.add_argument('-window_size', default=window_size_saved, type=int)
    parser.add_argument('-num_candidates', default=num_candidates_saved, type=int)
    ...
    with torch.no_grad():
        abnormal_block_ids, abnormal_log_ids, abnormal_behavior_sub_sequences, prodict_values, predict_pros = [], [], [], [], []
        for line_nl, line_bi, line_li in zip(test_normal_loader, block_ids, log_ids):
            for i in range(len(line_nl) - window_size):
                seq = line_nl[i:i + window_size] #检测位置前的输入的序列
                detect_position=i + window_size# 检测位置
                label = line_nl[detect_position]# 检测对象
                seq = torch.tensor(seq, dtype=torch.float).view(-1, window_size, input_size).to(device)
                label = torch.tensor(label).view(-1).to(device)
                output = model(seq)
                output_pro = torch.softmax(output, dim=1) #含有概率
                # print(output[0])
                predicted = torch.argsort(output, 1)[0][-num_candidates:]
                predicted_reversed = reversed(predicted) #概率大的排在前面
                predicted_value = []
                predicted_pro = [] #与predicted_value对应的概率值
                top_num = 3 #截取前几个数据
                for ele_p in predicted_reversed[0:top_num]:
                    predicted_value.append(ele_p.tolist()+1)
                    predicted_pro.append(f'{"{:.4f}".format(output_pro[0][ele_p]*100)}%')
                    #predicted_pro.append(f'{ele_p+1}:{"·{:.4f}".format(output_pro[0][ele_p]*100)}%') #前面减一这里加一
                # print(predicted_pro)
                #正例
                if label in predicted:
                    TP+=1
                else:# 假正例(负例)
                    abnormal_behavior_sub_sequence = [x for x in (int(x + 1) for x in
                                                                  (seq.flatten().tolist() + label.flatten().tolist()))
                                                      if x != 0]
                    ...
                    prodict_values.append(predicted_value)
                    predict_pros.append(predicted_pro)
                    FP += 1

        #根据日志块id进行合并
        abnormal_block_ids2 = list(OrderedDict.fromkeys(abnormal_block_ids).keys())
        abnormal_log_ids3, abnormal_behavior_sub_sequences3, predict_values3, predict_pros3= [], [], [], []
        abnormal_block_ids3 = abnormal_block_ids2
        for abnormal_block_id in abnormal_block_ids2:
            abnormal_log_ids2, abnormal_behavior_sub_sequences2, predict_values2, predict_pros2 = [], [], [], []
            ...
            for element_position in position:
                ...
                predict_values2.append(prodict_values[element_position])
                predict_pros2.append(predict_pros[element_position])
            abnormal_log_ids3.append(abnormal_log_ids2)
            abnormal_behavior_sub_sequences3.append(abnormal_behavior_sub_sequences2)
            predict_values3.append(predict_values2)
            predict_pros3.append(predict_pros2)
            #print(f"{abnormal_block_id}: {abnormal_log_ids2}: {abnormal_behavior_sub_sequences2}")

        # 存储异常数据
        df = pd.DataFrame({"abnormal_block_ids": abnormal_block_ids3, "abnormal_log_ids": abnormal_log_ids3,
                           "abnormal_behavior_sub_sequences": abnormal_behavior_sub_sequences3,
                           "predict_values": predict_values3,
                           "predict_pros": predict_pros3})
        df.to_csv(f"{get_parent_path()}/service/impl/tmpdata/Anormaly/behavior_sequence_anormal.csv", index=False,
                  encoding='utf-8')
        #print(df)
