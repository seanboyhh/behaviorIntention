...
from behavior_intention.cn.edu.xidian.sai.dao.impl.data_process_dao import csv_to_simple
...
def generate(name):
    ...
    #hdfs = set()
    hdfs, lis= [], []
    behavior_sequences, block_ids, log_ids=csv_to_simple(f'{get_parent_path()}/service/impl/deeplog/data/' + name)
...
def behavior_sequence_predict(num_classes = 125):
    # Hyperparameters
    num_classes = 125
    input_size = 1
    model_path = get_parent_path()+'/service/impl/tmpdata/SeqModel/Adam_batch_size=2048_epoch=1000.pt'
    ...
    test_normal_loader, block_ids, log_ids = generate('log_entry_to_behavior_block.csv')
    ...
    with torch.no_grad():
        ...
        df.to_csv(f"{get_parent_path()}/service/impl/tmpdata/behavior_sequence_anormal.csv", index=False, encoding='utf-8')
        #print(df)
