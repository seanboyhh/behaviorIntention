...
from behavior_intention.cn.edu.xidian.sai.dao.impl.data_process_dao import csv_to_simple3, get_first_file
...
def generate(name):
    ...
    #hdfs = set()
    hdfs = []
    behavior_sequences, block_ids=csv_to_simple3(f'{get_parent_path()}/service/impl/tmpdata/NextBehaviorBasedData/' + name)
    for line_bs in behavior_sequences:
...
def next_behavior_predict():
    ...
    test_normal_loader, block_ids = generate('new_behavior_sequence.csv')
    ...
    with torch.no_grad():
        abnormal_block_ids, abnormal_behavior_sub_sequences=[], []
        ...
        # 存储异常数据
        df=pd.DataFrame({"abnormal_block_ids": abnormal_block_ids3, "abnormal_behavior_sub_sequences": abnormal_behavior_sub_sequences3})
        df.to_csv(f"{get_parent_path()}/service/impl/tmpdata/Anormaly/next_behavior_sequence_anormal.csv", index=False, encoding='utf-8')
        #print(df)
